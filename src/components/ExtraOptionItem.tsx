...
const ExtraOptionItem = ({
  ...
  return <div className="flex justify-between items-center py-5 border-b last:border-b-0">
      <div className="class=\"pl-4\"\n">
        <div className="flex items-center">
          <h4 className="text-lg font-semibold">{extra.name}</h4>
          {extra.price > 0 && <span className="ml-2 px-2 py-1 bg-brand text-white rounded-md text-sm">
              ${extra.price}
            </span>}
        </div>
      ...
      <div className="flex items-center space-x-3">
        <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={handleDecrement} disabled={quantity === 0}>
          <Minus size={16} />
        </Button>
